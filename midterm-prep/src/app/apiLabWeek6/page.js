...
export default function DogPage() {
        ...
        if(data) {
            ...
            data.forEach((medium, index) => {
                let formattedMedia = medium.url ? (
                    <img src={medium.url} alt={medium.name} width="300" />
                ) : (
                ...
                const breedInfo = medium.breeds && medium.breeds.length > 0 ? (
                    <div>
                        <h2>{medium.breeds[0].name}</h2>
                        <p><strong>Temperament</strong>: {medium.breeds[0].temperament}</p>
                        <p><strong>Origin</strong>: {medium.breeds[0].origin || "Unknown"}</p>
                    </div>
                ) : (
    ...
    return (
        <>
            <div className="main">
                <header>
                    <h1>This is Monica's Sandbox for Doggos</h1>
                    <button onClick={fetchData} className="button-fetch">
                        Fetch Doggos 🐶
                    </button>
                </header>
